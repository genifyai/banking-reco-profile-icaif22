...
def getIndex(row, col):
    val = row[col].strip()
    if isinstance(val, float) or "." in val:
...
def processData(in_file_name, cust_dict):
    ...
    for row in csv.DictReader(in_file_name):
        ...
        cust_id = int(float(row['ncodpers']))
...
if __name__ == "__main__":
    ...
    preds = np.fliplr(preds)[:, :20]
    test_id = np.array(pd.read_csv(args.test_file, usecols=['ncodpers'])['ncodpers'])
    final_preds = [" ".join(list(target_cols[pred])) for pred in preds]
    out_df = pd.DataFrame({'ncodpers': test_id, 'added_products': final_preds})
    out_df.to_csv('data/rec_xgb.csv', index=False)
    print(datetime.datetime.now() - start_time)
