...
if __name__ == '__main__':
    """
    predictions: csv file containing the predictions given XGB model
    ...
    it outputs the metrics relative to the recommendations predicted by XGB model
    USAGE: python xgb_model/results.py --predictions data/rec_xgb.csv --ground_truth data/test_split.csv
    """
    parser = argparse.ArgumentParser()
    parser.add_argument('--predictions', type=str, required=True)
    parser.add_argument('--ground_truth', type=str, required=True)
    parser.add_argument('--history', type=str, default="data/train_split.csv")
    ...
    with open(predictions, 'r') as read_obj:
        csv_reader = reader(read_obj)
        header = next(csv_reader)
        for row in csv_reader:
            ...
            preds = row[1].split()
            if ownership:
                preds = [config.target_cols.index(p) for p in preds]
            else:
                preds = [config.target_cols.index(p) for p in preds if
                         config.target_cols.index(p) not in users[user_id].ownership]
            users[user_id].recommendations = preds
