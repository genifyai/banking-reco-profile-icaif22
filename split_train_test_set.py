...
if __name__ == '__main__':
    ...
    - the first one contains the data associated to the N-1 timestamps (used by AP and XGBoost to predict recommendations)
    - the second one contains the data associated to last timestamp (used for evaluation)
    USAGE: python split_train_test_set.py --input_file data/train_reduced.csv
    """
    ...
    df_test.to_csv(os.path.join("data", "train_split.csv"), index=False)
    df_pred.to_csv(os.path.join("data", "test_split.csv"), index=False)
    print("process done")
